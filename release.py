...
def get_latest_tag():
    ...
    latest_tag = sorted(
        tags,
        key=lambda t: tuple(map(int, re.match(r"v(\d+)\.(\d+)\.(\d+)", t).groups())),
    )[-1]
    return latest_tag
...
def update_version_number(latest_tag, increment):
    major, minor, patch = map(
        int, re.match(r"v(\d+)\.(\d+)\.(\d+)", latest_tag).groups()
    )
    if increment == "X":
        major += 1
        minor, patch = 0, 0
