...
def rerank_documents(
    ...
    """
    try:
        if rerank_model in list_rerank_models():  # 使用本地排序模型
            from server.utils import load_local_reranker

            reranker = load_local_reranker(model=rerank_model)
            rerank_results = reranker.rerank(query, [i[0].page_content for i in docs])
            rerank_docs = []
            if rerank_results.get("rerank_passages"):
                rerank_docs = [
                    docs[ids] + (score,)
                    for ids, score in zip(
                        rerank_results["rerank_ids"][:rerank_top_k],
                        rerank_results["rerank_scores"][:rerank_top_k],
                    )
                    if score >= rerank_threshold
                ]
            return rerank_docs

        return BaseResponse(code=500, msg=f"指定的模型 {rerank_model} 不支持排序功能。")
    except Exception as e:
        logger.error(e)
        return BaseResponse(code=500, msg=f"排序过程错误：{e}")
